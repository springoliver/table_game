...
document.getElementById("resetBtn").addEventListener("click", () => {
  if (!confirm("Reset all tables and lists?")) return;
...
function renderTables() {
  ...
  state.tables.forEach((table) => {
    ...
    seatOrder.forEach((seatIndex, orderIndex) => {
      ...
      pill.textContent = seat.player
        ? `${orderIndex + 1}. ${seat.player.name}`
        : `${orderIndex + 1}. Empty`;
...
function normalizeDaisyTypes() {
  ...
  state.tables.forEach((table, index) => {
    table.type = index === state.tables.length - 1 ? "mm" : "main";
  });
  state.tables.forEach((table, index) => {
    table.daisyNextId = index < state.tables.length - 1 ? state.tables[index + 1].id : null;
  });
}
